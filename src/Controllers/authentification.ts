...
export default class AuthController {
  ...
  static async signup(req: Request, res: Response) {
    ...
    try {
      ...
      qr = "INSERT INTO `User` (`user_id`, `role`, `username`, `firstname`, `lastname`, `email`, `password`, `adress`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
      let [row] = await conn.query<ResultSetHeader>(qr, [user_id, role, username, firstname, lastname, email, hashedPassword, adress]);
      ...
      const access_token = jwt.sign(user, access_token_secret, { expiresIn: "30d" });
      const refresh_token = jwt.sign(user, refresh_token_secret, { expiresIn: "30d" });
